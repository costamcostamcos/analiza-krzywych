...
if uploaded_file:
    ...
    # PLOTLY WYKRES 1
    st.subheader("Wykres 1: Interaktywne krzywe")
    fig = go.Figure()
    ...
    # Inicjalizacja tablicy MSE o rozmiarze danych
    mse_vals = np.zeros(dane.shape[0])
    ...
    for k_id in np.unique(numery_grup):
        ...
        if any(maska):
            ...
            # MSE względem centroidu
            dane_k = dane[maska]
            centroid = dane_k.mean(axis=0)
            mse_vals[maska] = np.mean((dane_k - centroid)**2, axis=1)
            
    st.plotly_chart(fig_s, use_container_width=True)

    # MSE DIAGNOSTYKA - przypisanie do nowej kolumny bezpiecznie
    df_diagnostyka = pd.DataFrame({'Krzywa': krzywe.columns, 'MSE': mse_vals})
    st.markdown("##### 🚨 Top 5 Anomalii (Najwyższe MSE):")
    st.dataframe(df_diagnostyka.sort_values('MSE', ascending=False).head(5), use_container_width=True)
