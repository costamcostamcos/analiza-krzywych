...
# =================================================================
# NOWOŚĆ: GENERATOR MATEMATYCZNEJ AUGMENTACJI KRZYWYCH (DLA DEEP LEARNINGU)
# =================================================================
def augmentuj_dane(X_oryginalne, czynniki_kopii=50, noise_level=0.02, scale_range=0.05):
    """
    Namnaża zbiór danych w sposób fizycznie bezpieczny za pomocą trzech transformacji:
    Jittering (szum), Scaling (skalowanie amplitudy), Time-Warping (lekkie zniekształcenie osi czasu)
    """
    N, F = X_oryginalne.shape
    X_namnozone = []
    
    # Zachowujemy oryginalne dane na początku zbioru
    X_namnozone.append(X_oryginalne)
    
    for c in range(czynniki_kopii - 1):
        kopia = np.copy(X_oryginalne)
        
        # 1. Jittering (Dodanie losowego szumu aparatury)
        szum = np.random.normal(0, noise_level, size=kopia.shape)
        kopia += szum
        
        # 2. Scaling (Wahania globalnej amplitudy sygnału)
        skala = np.random.uniform(1.0 - scale_range, 1.0 + scale_range, size=(N, 1))
        kopia *= skala
        
        # 3. Mini Time-Warping (Subtelne nieliniowe zniekształcenia osi czasu/X)
        for i in range(N):
...
if df is not None:
    try:
        ...
        if metoda == "K-means":
            ...
            with st.spinner("🧠 🧠 Trwa matematyczna augmentacja danych (44 -> 2200) oraz głęboki trening DEC..."):
                # Wywołanie nowej funkcji augmentacji w celu stabilizacji enkodera
                ...
                # Uczenie na wielkim zbiorze zaszumionym (Augmented Dataset)
                ...
                # Finalne kodowanie cech oryginalnych krzywych laboratoryjnych
                net.eval()
                with torch.no_grad():
            ...
            with st.spinner("🛡️ 🛡️ Trwa trening RDEC z barierą L2 zasilany bazą 2200 prób..."):
                ...
                net = AutoencoderKrzywych(input_dim=input_dim, latent_dim=4)
                criterion = nn.MSELoss()
                # weight_decay realizuje regularyzację L2
    ...
    st.info("💡 Aby rozpocząć, wgraj plik z dysku lub wklej link do Google Sheets powyżej.")import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
from sklearn.preprocessing import StandardScaler, MinMaxScaler
from sklearn.cluster import KMeans, HDBSCAN, SpectralClustering
